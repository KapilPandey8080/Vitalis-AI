...
// --- Main HDiseasePredict Component ---
const HDiseasePredict = () => {
    ...
    const featureLabels = [
        'Age', 'Chest Pain Type (0-3)', 'Resting BP', 'Cholesterol', 'Max Heart Rate',
        'Exercise Induced Angina (0-1)', 'ST Depression (Oldpeak)', 'Slope (0-2)',
        'Major Vessels (0-3)', 'Thalassemia (1-3)'
    ];
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post("http://127.0.0.1:5000/heart_disease_predict", { input_data });

            setResult({
                message: response.data.message,
                highRisk: response.data.probability_heart_disease,
                healthy: response.data.probability_healthy
            });
        } catch (error) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
                <Text style={styles.subtitle}>Enter the patient's cardiac metrics below.</Text>
                ...
                <TouchableOpacity
                    ...
                    disabled={isLoading}
                >
                    {isLoading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.buttonText}>Check Heart Risk</Text>}
                </TouchableOpacity>

                {result && (
                    <Animated.View style={[styles.resultCard, {
                        borderColor: result.message.includes('has Heart Disease') ? theme.danger : theme.success,
                        backgroundColor: result.message.includes('has Heart Disease') ? '#FFF1F1' : '#F0FFF0'
                    }]}>
                        <Text style={styles.resultTitle}>Risk Assessment</Text>
                        <Text style={[styles.resultText, { color: result.message.includes('has Heart Disease') ? theme.danger : theme.success }]}>
                            {result.message}
                        </Text>
                        
                        <View style={styles.probabilityContainer}>
                            <View style={styles.probRow}>
                                <Text style={styles.probLabel}>Confidence (High Risk)</Text>
                                <Text style={[styles.probValue, {color: theme.danger}]}>{(result.highRisk * 100).toFixed(1)}%</Text>
                            </View>
                            <View style={styles.progressBar}>
                                <View style={[styles.progressFill, { width: `${result.highRisk * 100}%`, backgroundColor: theme.danger }]} />
                            </View>
                        </View>
                        ...
                        <View style={styles.noteContainer}>
                            ...
                            <Text style={styles.noteText}>This is an AI-generated assessment and not a medical diagnosis. Always consult a healthcare professional.</Text>
                        </View>
                    </Animated.View>
                )}
            </ScrollView>
...
export default HDiseasePredict;
